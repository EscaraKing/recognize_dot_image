...
from tkinter import Scale, Button, Label, Radiobutton, filedialog, Spinbox, Checkbutton, Text, END, ttk
...
scrollbar_main = ttk.Scrollbar(outer, orient="vertical", command=canvas.yview)
...
def _on_configure(_event=None):
    # inner 내용 크기에 따라 스크롤 영역 업데이트
    canvas.configure(scrollregion=canvas.bbox("all"))
inner.bind("<Configure>", _on_configure)

def _on_mousewheel(event):
    # Windows(+120/-120). macOS는 delta 단위가 다를 수 있음.
    canvas.yview_scroll(int(-1*(event.delta/120)), "units")
canvas.bind_all("<MouseWheel>", _on_mousewheel)

# ---- 로그 영역 (전용 스크롤바) ----
log_text = Text(inner, height=10)
...
def analyze_all():
    ...
    if write_header:
        meta_writer.writerow([
            ...
            "box_x1","box_y1","box_y2","box_x2",  # (주의) x2/y2 순서 수정 가능
            "center_x","center_y"
        ])
        # 위 컬럼 순서는 기존 사용 코드(pairing)와 맞춰주세요.
        # 만약 기존 순서를 사용 중이라면 아래 작성에서 순서 동일하게 기록하세요.
    ...
    for fname in image_files:
        ...
        for ci, idx_list in enumerate(clusters):
            ...
            meta_writer.writerow([
                fname, ci,
                os.path.basename(grid_path), grid_y, grid_x,
                int(min_x), int(min_yc), int(max_yc), int(max_x),  # 위의 헤더 순서에 맞춤
                cx, cy
            ])
...
# ===================== UI 구성 (inner 프레임에 모두 배치) =====================
row = 0
Label(inner, text="ROI - min_y").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=0, to=(img_height-1 if img is not None else 1), orient='horizontal',
      variable=min_y_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="ROI - max_y").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=0, to=(img_height-1 if img is not None else 1), orient='horizontal',
      variable=max_y_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="Threshold Type").grid(row=row, column=0, sticky="w", padx=8, pady=4)
frame_t = tk.Frame(inner); frame_t.grid(row=row, column=1, sticky="w", padx=8, pady=4)
...
row += 1
Label(inner, text="threshold (Binary계열)").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=0, to=255, orient='horizontal', variable=thresh_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="Adaptive block size (odd)").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=3, to=51, orient='horizontal', variable=adaptive_block_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="Adaptive C").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=-20, to=20, orient='horizontal', variable=adaptive_c_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="min_area").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=1, to=5000, orient='horizontal', variable=min_area_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="max_area").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=10, to=10000, orient='horizontal', variable=max_area_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="min_circularity").grid(row=row, column=0, sticky="w", padx=8, pady=4)
scale_min_circ = Scale(inner, from_=0, to=100, orient='horizontal',
      variable=tk.IntVar(value=int(min_circularity_var.get()*100)),
      command=lambda v: (min_circularity_var.set(float(v)/100), update_image()))
scale_min_circ.grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="max_circularity").grid(row=row, column=0, sticky="w", padx=8, pady=4)
scale_max_circ = Scale(inner, from_=0, to=100, orient='horizontal',
      variable=tk.IntVar(value=int(max_circularity_var.get()*100)),
      command=lambda v: (max_circularity_var.set(float(v)/100), update_image()))
scale_max_circ.grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="Cluster gap eps (px)").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=1, to=200, orient='horizontal', variable=cluster_eps_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Label(inner, text="Cluster min points").grid(row=row, column=0, sticky="w", padx=8, pady=4)
Scale(inner, from_=1, to=20, orient='horizontal', variable=cluster_min_pts_var, command=update_image).grid(row=row, column=1, sticky='we', padx=8, pady=4)

row += 1
Checkbutton(inner, text="Auto grid from bbox ratio (axis ≤ 6)", variable=auto_grid_var,
            command=update_image).grid(row=row, column=0, columnspan=2, sticky="w", padx=8, pady=(8,4))

row += 1
Label(inner, text="Grid size (X cells / cols)").grid(row=row, column=0, sticky="w", padx=8, pady=4)
grid_x_spin = Spinbox(inner, from_=2, to=64, textvariable=grid_x_var, width=8, command=update_image)
grid_x_spin.grid(row=row, column=1, sticky="w", padx=8, pady=4)

row += 1
Label(inner, text="Grid size (Y cells / rows)").grid(row=row, column=0, sticky="w", padx=8, pady=4)
grid_y_spin = Spinbox(inner, from_=2, to=64, textvariable=grid_y_var, width=8, command=update_image)
grid_y_spin.grid(row=row, column=1, sticky="w", padx=8, pady=4)

row += 1
Button(inner, text="분석 시작 (그리드/메타 저장)", command=analyze_all).grid(row=row, column=0, sticky="w", padx=8, pady=8)

# ---- 로그: Text + 전용 스크롤바 ----
row += 1
Label(inner, text="Logs").grid(row=row, column=0, sticky="w", padx=8, pady=(8,4))
log_frame = ttk.Frame(inner)
log_frame.grid(row=row, column=1, columnspan=1, sticky="nsew", padx=8, pady=(8,4))
log_text = Text(log_frame, height=16, wrap="word")
